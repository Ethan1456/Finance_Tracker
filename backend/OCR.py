...
def extract_items(path):
    ...
    # item dictionary
    itemDict = {}
    skip_keywords = ["GRAND TOTAL", "TOTAL", "TAX", "SUBTOTAL", "CASH", "CHANGE"]
    for line in text.splitlines():
        if any(k in line.upper() for k in skip_keywords):
            continue
        
        match = re.match(r'^\s*(?:(\d+)\s+)?([A-Za-z][A-Za-z\s]+?)\s+\$?(\d+\.\d{2})\s*$', line)
        
        if match:
            name = match.group(2).strip()
            price = float(match.group(3))  # treat as total for line
            # Option 1: set quantity to 1 because we ignore it
            itemDict[name] = {"quantity": 1, "price": price}
